...
class TestNotebookDocumentModel:
    ...
    def test_empty_notebook_roundtrip(self) -> None:
        ...
        assert nb2.session_id == "s1"
        assert nb2.created_at == "2026-01-01T00:00:00Z"
        assert len(nb2.cells) == 0
        assert raw["nbformat"] == 4
        assert raw["nbformat_minor"] == 5
    ...
    def test_code_cell_roundtrip(self) -> None:
        ...
        c = nb2.cells[0]
        assert c.id == "c1"
        assert c.source == "x = 1 + 2"
        assert c.metadata.purpose == "test"
        assert c.metadata.produces == ["x"]
        assert c.execution_count == 1
        assert c.outputs[0].data["text/plain"] == "3"
    ...
    def test_markdown_cell_roundtrip(self) -> None:
        ...
        assert nb2.cells[0].source == "# Hello\n\nWorld"
        assert nb2.cells[0].metadata.tags == ["preamble"]
    ...
    def test_error_output_roundtrip(self) -> None:
        ...
        assert out.evalue == "division by zero"
    ...
    def test_cell_lookup_helpers(self) -> None:
        ...
        assert nb.cell_by_id("a") is c1
        assert nb.cell_by_id("z") is None
        assert nb.cell_index("b") == 1
        assert nb.cell_index("z") is None
...
class TestNotebookStore:
    ...
    def _setup(self, tmp_path: Path) -> None:
        self.store = NotebookStore(tmp_path)
    ...
    def test_load_nonexistent_returns_empty(self) -> None:
        ...
        assert nb.session_id == "nonexistent"
    ...
    def test_append_cell(self) -> None:
        ...
        assert nb2.cells[0].source == "y=2"

    def test_delete(self) -> None:
        self.store.create_empty("s1")
        assert self.store.exists("s1")
        self.store.delete("s1")
        assert not self.store.exists("s1")
...
class TestNotebookOrchestrator:
    ...
    def test_update_cell(self) -> None:
        ...
        assert nb.cells[0].id == "c1"  # id preserved
    ...
    def test_delete_source_binding_fails(self) -> None:
        ...
        assert not result.ok
        assert "source_binding" in result.error
    ...
    def test_move_cell(self) -> None:
        ...
        nb = self.store.load("s1")
        assert [c.id for c in nb.cells] == ["c", "a", "b"]
    ...
    def test_execute_records_outputs(self) -> None:
        ...
        assert nb.cells[0].outputs[0].data["text/plain"] == "1"
...
class TestSessionSource:
    ...
    def test_source_roundtrip(self) -> None:
        ...
        assert src2.alias == "sales_csv"
        assert src2.variable_name == "sales_df"
        assert src2.schema_hint["amount"] == "float64"
    ...
    def test_manifest_multi_source(self) -> None:
        ...
        assert m.has_csv()
        assert m.has_db()
        assert m.source_by_alias("csv1") is not None
        assert m.source_by_alias("nonexistent") is None

    def test_manifest_primary_source(self) -> None:
        m = SessionManifest(session_id="s1")
        assert m.primary_source() is None

        m.add_source(SessionSource(alias="csv1", source_type="csv"))
        assert m.primary_source().alias == "csv1"
    ...
    def test_manifest_add_source_replaces_existing_alias(self) -> None:
        ...
        assert len(m.sources) == 1
        assert m.sources[0].display_name == "v2"
    ...
    def test_manifest_roundtrip(self) -> None:
        ...
        assert m2.selected_skill_ids == ["skill1"]
...
class TestAliasGeneration:
    ...
    def test_csv_alias(self) -> None:
        assert make_source_alias("Sales Q4.csv", "csv", []) == "sales_q4_csv"

    def test_db_alias(self) -> None:
        assert make_source_alias("Warehouse", "db_connection", []) == "warehouse_db"

    def test_deduplication(self) -> None:
        existing = ["sales_q4_csv"]
        assert make_source_alias("Sales Q4.csv", "csv", existing) == "sales_q4_csv_2"

    def test_triple_dedup(self) -> None:
        existing = ["sales_csv", "sales_csv_2"]
        assert make_source_alias("sales.csv", "csv", existing) == "sales_csv_3"

    def test_empty_name(self) -> None:
        alias = make_source_alias("", "csv", [])
        assert alias == "source_csv"

    def test_variable_name_csv(self) -> None:
        assert alias_to_variable_name("sales_csv") == "sales_df"

    def test_variable_name_db(self) -> None:
        assert alias_to_variable_name("warehouse_db") == "warehouse_conn"

    def test_variable_name_other(self) -> None:
        assert alias_to_variable_name("custom") == "custom_data"
...
class TestCellBuilder:
    ...
    def test_build_from_tool_result(self) -> None:
        ...
        assert cell.metadata.tool_name == "pandas_tool"
        assert cell.metadata.purpose == "Aggregate revenue by region"
        assert cell.metadata.produces == ["agg"]
        assert "df" in cell.metadata.depends_on  # extracted from AST
        assert cell.execution_count == 3
        assert "analysis" in cell.metadata.tags

        # Verify breadcrumb comments in source.
        assert "# PURPOSE: Aggregate revenue by region" in cell.source
        assert "# PRODUCES: agg" in cell.source
        assert "# DEPENDS_ON:" in cell.source
    ...
    def test_build_plotly_cell(self) -> None:
        ...
        assert "visualization" in cell.metadata.tags
        assert "monthly" in cell.metadata.depends_on
    ...
    def test_build_source_binding_cell(self) -> None:
        ...
        assert cell.is_source_binding
        assert cell.metadata.source_alias == "sales_csv"
        assert cell.metadata.produces == ["sales_df"]
        assert cell.metadata.created_by == "system"
        assert "# PURPOSE: Load source: Sales Q4" in cell.source

    def test_build_preamble_cell(self) -> None:
        cell = build_preamble_cell("s1", "Sources: sales.csv")
        assert cell.cell_type == "markdown"
        assert "preamble" in cell.metadata.tags
        assert "Sources: sales.csv" in cell.source
    ...
    def test_depends_on_extracts_variable_refs(self) -> None:
        ...
        assert "sales_df" in deps
        assert "orders_df" in deps
        # 'result' should NOT be in depends_on (it's stored, not loaded).
        assert "result" not in deps
...
class TestFullPipelineE2E:
    ...
    @pytest.fixture(autouse=True)
    def _setup(self, tmp_path: Path) -> None:
        self.nb_store = NotebookStore(tmp_path)
        self.mf_store = ManifestStore(tmp_path)
    ...
    def test_full_session_lifecycle(self) -> None:
        ...
        assert "preamble" in nb.cells[0].metadata.tags

        # Source binding (CSV)
        assert nb.cells[1].is_source_binding
        assert nb.cells[1].metadata.source_alias == "sales_csv"
        assert nb.cells[1].metadata.produces == ["sales_df"]

        # Analysis cell
        assert nb.cells[2].metadata.tool_name == "pandas_tool"
        assert nb.cells[2].metadata.produces == ["monthly"]
        assert "sales_df" in nb.cells[2].metadata.depends_on
        assert nb.cells[2].metadata.artifact_refs == ["art-001"]
        assert "# PURPOSE: Monthly revenue aggregation" in nb.cells[2].source

        # Visualization cell
        assert nb.cells[3].metadata.tool_name == "plotly_tool"
        assert "visualization" in nb.cells[3].metadata.tags
        assert "monthly" in nb.cells[3].metadata.depends_on

        # Source binding (DB)
        assert nb.cells[4].is_source_binding
        assert nb.cells[4].metadata.source_alias == "warehouse_db"
        ...
        assert m.has_csv()
        assert m.has_db()
        assert m.primary_source().alias == "sales_csv"
        ...
        assert len(nb_restored.cells) == 5
        assert nb_restored.cells[2].metadata.tool_name == "pandas_tool"
    ...
    def test_source_removal_does_not_delete_binding_cell(self) -> None:
        ...
        nb = self.nb_store.load(sid)
        assert len(nb.cells) == 1
        assert nb.cells[0].is_source_binding


# ── 8. KernelManager restore test ───────────────────────────────────────────


class TestKernelManagerRestore:
    ...
    def _setup(self, tmp_path: Path):
        ...
        session_dir = base / "sessions" / self.session_id / "sources"
    ...
    def test_restore_replays_cells(self) -> None:
        ...
        assert len(state.restore_errors) == 0
        assert state.cell_execution_count == 2  # source_binding + analysis cell
