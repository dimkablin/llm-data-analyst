...
from backend.agent.models import AgentOutcome, AgentResponse, CapabilityOutcome, ErrorCategory
...
def finalize_node(
    ...
    if response is None:
        ...
        text = (
            ...
            else artifact_summary_text(artifacts) or build_fallback_text(prompt, df, stop_reason=stop_reason)
        )
        response = AgentResponse(
            final_text=text,
            reasoning=(
                "LLM invoke failed" if state.get("llm_unreachable") else "No response produced by graph."
            ),
            artifacts=artifacts,
            route="analysis",
            ...
            outcome=AgentOutcome.failed(
                ErrorCategory.MODEL if state.get("llm_unreachable") else ErrorCategory.INTERNAL
            ),
        )
    else:
        ...
        if getattr(response, "llm_unreachable", False):
            ...
            response.final_text = artifact_summary_text(
                list(response.artifacts or [])
            ) or build_fallback_text(prompt, df, stop_reason=stop_reason)

    response.capability_outcomes = _capability_outcomes(
        callbacks,
...
def _capability_outcomes(
    ...
    *,
    ...
    by_tool = {
        str(item.get("bound_tool_key") or ""): item
        for item in capability_context.get("capabilities", [])
        if isinstance(item, dict) and item.get("bound_tool_key")
    }
    ...
    for callback in callbacks:
        for event in getattr(callback, "events", []) or []:
            if event.get("phase") != "end":
                continue
            tool_key = str(event.get("tool_name") or "").strip()
            capability = by_tool.get(tool_key)
            if not isinstance(capability, dict):
                continue
            outcomes.append(
                CapabilityOutcome(
                    capability_key=str(capability.get("key") or ""),
                    tool_key=tool_key,
                    status="error" if event.get("status") == "error" else "ok",
                    artifact_types=[
                        str(item) for item in (event.get("artifact_keys") or []) if str(item).strip()
                    ],
                    provenance=str(capability.get("provenance") or ""),
                )
            )
    return outcomes
