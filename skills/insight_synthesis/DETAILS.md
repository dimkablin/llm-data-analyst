...
### Step 1 — collect key metrics → pandas_tool
```python
...
for col in num_cols[:8]:
    ...
    if cv > 0.01:  # skip constants
        ...
        if abs(s.mean() - s.median()) / (abs(s.median()) + 1e-9) > 0.2:
            row["alert"] = f"mean/median diverge by {abs(s.mean()-s.median())/abs(s.median())*100:.0f}%"
        metric_rows.append(row)

key_metrics = pd.DataFrame(metric_rows)
tool_result = {"schema_version": "1.0", "artifact_type": "table", "items": {"key_metrics": key_metrics}}
tool_result
```
